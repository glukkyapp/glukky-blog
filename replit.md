...
8. **Next-Day Adjustment**: Only applies when tomorrow has a walk scheduled. Priority order (higher overrides lower): (1) Today is stretch + completed (regardless of tired) → tomorrow back to baseline walk (`walkDurationGoal`, `adjustedToStretch=false`). (2) Today is stretch + fail + NOT tired → tomorrow back to baseline walk. (3) Today is stretch + fail + tired → tomorrow stretch again. (4) 2 consecutive fail+tired days → tomorrow stretch (overrides single-day reduction). (5) Single fail+tired → tomorrow `walkDurationGoal - 5`. (6) Otherwise → no change. Reduction always subtracts from `plan.walkDurationGoal` (not from stored day duration). Walking Bridge (0 walks over 2 weeks) counts only real walk days, NOT fatigue stretch days. Weekly report excludes stretch-adjusted days from walk percentage and adds "Stretching: X day(s)" remark.
