...
5. **Diet Struggle System**: Sugary Food/Drink → Oily/Fried Food → Eat Out → Portions → Snacks. **Struggles** (not tips) are mastered over 3-week cycles. The 3 weeks do NOT need to be consecutive — only weeks where the diet struggle was actually active (plan's `dietStruggle` matches `profile.currentStruggle`) count toward the cycle. Late dinner override weeks (where `dietStruggle` is null) do NOT count. Each week, user chooses which tip to practice from the struggle's ladder (if only 1 tip, auto-assigned). Subsequent weeks: "keep same tip?" → yes skips selection, no → shows all tips. After 3 diet-active weeks: ≥16 yes days → master struggle; ≥11 no_chance days → skip struggle (move to bottom of list); neither → stay for 3 more weeks; 2 consecutive stays → move on (move to bottom). Both skip and move_on put struggle at bottom of struggles array. `tipCycleStartWeek` and `tipStayCycles` track cycle progress on user_profiles. `selectedTip` sent from planner → stored as plan's `dietTip`. **Diet evaluation** is computed server-side via `evaluateDietStruggle()` in engine.ts (read-only helper) and returned in the reflection response as `dietEvaluation: { type, nextStruggle }`. Frontend uses this directly — no client-side graduation logic. Profile updates (moving to next struggle) happen in `POST /api/plan/weekly` using the same helper.
6. **Bi-Weekly Triggers**: Walking Bridge (0 walks over 2 weeks → enter stretch mode), Auto-Escalation (100% stretch for 2 consecutive weeks → offer 10-min walks, requires `stretchSuccessWeeks >= 2`), Stagnation Pivot
7. **Walking Bridge / Stretch Mode**: When `isStretchMode=true`, walk day picker relabels to "stretch days" with 2-min duration. `stretchSuccessWeeks` tracks consecutive 100% stretch weeks. After 2 → auto-escalation offer (10-min walks). Calendar/check-in labels say "stretch" instead of "walk". Empty week offers stretch via `stretchOffer` step. `stretchOnly` flag in plan creation overrides walk durations to 2 without changing profile baseline. Escalation Yes: sets `acceptedEscalation=true`, switches UI to walk labels/duration selector, sends `stretchOnly: false`. Escalation No: keeps stretch UI. Weekly report: full stretch mode weeks show "X stretch days" instead of "X/Y walk days". `stretchSuccessWeeks` off-by-one fix: uses `updatedStretchSuccessWeeks` after increment in same request.
...
10. **Next-Day Adjustment**: Only runs when the current POST includes `walkCompleted` or `walkTired` (diet/dinner updates do NOT trigger it). Only applies when tomorrow has a walk scheduled AND today is NOT a standing tap day AND tomorrow is NOT a standing tap day. Priority order (higher overrides lower): (1) Today is stretch + completed → clear stretch flag (`adjustedToStretch=false`), keep tomorrow's original per-day duration. (2) Today is stretch + fail + NOT tired → clear stretch flag, keep per-day duration. (3) Today is stretch + fail + tired → tomorrow stretch again. (4) 2 consecutive fail+tired days → tomorrow stretch (overrides single-day reduction). (5) Single fail+tired → tomorrow `tomorrowDay.walkDuration - 5` (min 5). (6) Otherwise → no change. All duration logic uses per-day `walkDuration` directly, no fallback to `walkDurationGoal`. Walking Bridge (0 walks over 2 weeks) counts only real walk days, NOT fatigue stretch days. Weekly report excludes stretch-adjusted days from walk percentage and adds "Stretching: X day(s)" remark.
