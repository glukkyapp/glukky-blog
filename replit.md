...
**Self-healing & verdict badges (Task #486):** The paywall captures the anonymous RC subscriber id through a 4-step fallback sequence (purchase callback → `getCustomerInfo` → `getAppUserID` → `getAppUserId`, 4s per step). Captured ids are persisted to the `subscription_alias` table (anonymousAppUserId → replitUserId) on every successful alias call so a subsequent `/api/refresh-premium-status` can re-verify against RC even if the in-memory cache was wiped by a server restart. Server-side verification (`verifyEntitlementSelfHealing`) tries the user's direct entitlement first, then walks every persisted alias for that user before answering — `aliasGranted: true` in the trace marks an unlock that only succeeded via this fallback. Every paywall trace event carries an `installId` (UUID stored under `glukky.installId` in localStorage) and ends with a `verdictBadge` of one of: `OK` / `AUTH_NOT_READY` / `BRIDGE_MISSING_PURCHASE` / `RC_ID_NEVER_OBTAINED` / `MAPPING_POST_FAILED` / `VERIFY_AFTER_MAPPING_FAILED` / `OTHER` — grep deployment logs by badge to triage failure modes without parsing free-form messages.
